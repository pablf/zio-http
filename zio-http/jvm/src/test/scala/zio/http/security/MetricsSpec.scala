...
object MetricsSpec extends ZIOHttpSpec {
        ...
        test(name) {
          for {
            ...
            _ <- ZIO.iterate((0, init))( _._1 < maxReq){ case (n, content) =>
              ( ZIO.scoped{ Client.request(mkRequest(content)).flatMap { req =>
  ...
  val spec: Spec[TestEnvironment with Scope, Any]  = suite("MetricsSpec")(
    testMetrics("infinite url", 2000, "A"* 3000, _ ++ "A", port => (path => Request.get(s"http://localhost:$port/$path"))),
    testMetrics("infinite small segments url", 1000, "/A"* 1500 , _ ++ "/A", port => (path => Request.get(s"http://localhost:$port$path"))),
    testMetrics("infinite header", 2000, "A"* 7000, _ ++ "A", port => (header => Request.get(s"http://localhost:$port").addHeader(Header.Custom("n", header)))),
    testMetrics[List[Header.Custom]]("infinite small segments header", 1000, (0 until 1000).toList.map(s => Header.Custom(s"$s", "A")), (l: List[Header.Custom]) => (Header.Custom(l.head.customName.toString ++ "A", "A") :: l), port => (headers => Request.get(s"http://localhost:$port").addHeaders(Headers(headers:_*)))),
    testMetrics("infinite body", 10, "A"* (1023*100), _ ++ "A", port => (body => Request.post(s"http://localhost:$port", Body.fromString(body)))),
    testMetrics[Form]("infinite multi-part form", 10, Form(Chunk.fill(1300)(FormField.Simple("n", "A"))), _ + FormField.Simple("n", "A"), port => (form => Request.post(s"http://localhost:$port", Body.fromMultipartForm(form, Boundary("-"))))),
  ).provide(
      Server.customized,
