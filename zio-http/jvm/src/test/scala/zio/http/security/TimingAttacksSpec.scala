...
object TimingAttacksSpec extends ZIOSpecDefault {
  ...
  def boxTest2[A](h: () => Handler[Any, Nothing, Request, Response], slow: Request, fast: Request): ZIO[Any, Throwable, Boolean] = 
    ZIO.attempt {
  ...
  def suiteFor(name: String)(app: () => Handler[Any, Nothing, Request, Response], goodRequest: Request, almostGoodRequest: Request, badRequest: Request, sameLengthRequest: Request) =
    suite(name)(
  ...
  val spec = suite("TimingAttackSpec")(
    ...
    suiteFor("bearerAuthZIO")(bearerAuthZIOApp _, goodReqBearer, almostGoodReqBearer, badReqBearer, sameLengthReqBearer),
